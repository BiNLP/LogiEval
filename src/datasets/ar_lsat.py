...
class ARLSATDataset(BaseDataset):
    """AR-LSAT dataset from metaeval/reclor"""
    ...
    def load_data(self) -> None:
        """Load olegbask/AR-LSAT dataset from HuggingFace"""
        try:
            # import pdb;pdb.set_trace()  # Debugging breakpoint
            dataset = load_dataset("olegbask/AR-LSAT", split=self.split)
        except Exception as e:
            print(f"Error loading olegbask/AR-LSAT dataset: {e}")
        ...
        print(f"Loaded {len(self.examples)} examples from AR-LSAT {self.split} split")
